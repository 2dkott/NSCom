...
public class DecalBuilderLegacy {
	...
	public static void BuildDecalForObject(Decal decal, GameObject affectedObject) {
		...
		for(int i=0; i<triangles.Length; i+=3) {
			...
			DecalPolygonLegacy poly = new DecalPolygonLegacy( v1, v2, v3 );

			poly = DecalPolygonLegacy.ClipPolygon(poly, right);
			if(poly == null) continue;
			poly = DecalPolygonLegacy.ClipPolygon(poly, left);
			if(poly == null) continue;

			poly = DecalPolygonLegacy.ClipPolygon(poly, top);
			if(poly == null) continue;
			poly = DecalPolygonLegacy.ClipPolygon(poly, bottom);
			if(poly == null) continue;

			poly = DecalPolygonLegacy.ClipPolygon(poly, front);
			if(poly == null) continue;
			poly = DecalPolygonLegacy.ClipPolygon(poly, back);
			if(poly == null) continue;
	...
	private static void AddPolygon(DecalPolygonLegacy poly, Vector3 normal) {
